<template>
  <div id="header">
    <div class="nav-global">
      <div class="container">
        <h1 class="nav-logo">
          <a href="javascript:;"></a>
        </h1>
        <ul class="nav-aside">
          <li class="nav-user">
            <template v-if="ok">
              <router-link to="/account">个人中心</router-link>
              <div class="nav-user-wrapper">
                <div class="nav-user-list">
                  <dl class="nav-user-avatar">
                    <dd><span class="ng-scope"></span></dd>
                    <dt class="ng-binding" v-html="name"></dt>
                  </dl>
                  <ul>
                    <li class="order">
                      <router-link to="/account" exact>我的订单</router-link>
                    </li>
                    <li class="support">
                      <router-link to="/myCollection" exact>我的收藏</router-link>
                    </li>
                    <!--   <li class="coupon"><a href="javascript:;">我的优惠</a></li>
                      <li class="information"><a href="javascript:;">账户资料</a></li>-->
                    <li class="address">
                      <router-link to="/address">收货地址</router-link>
                    </li>
                    <li class="logout"><a href="javascript:;" @click="logout">退出</a></li>
                  </ul>
                </div>
              </div>
            </template>
            <template v-else>
              <router-link to="/login">登陆</router-link>
            </template>

            <!--active-->

          </li>
          <!--active-->
          <car-panel class="nav-cart" ref="carPanel"></car-panel>
        </ul>
        <ul class="nav-list">
          <li><a href="javascript:;">在线商城</a></li>
          <li><a href="javascript:;">坚果 Pro</a></li>
          <li><a href="javascript:;">Smartisan M1 / M1L</a></li>
          <li><a href="javascript:;">Smartisan OS</a></li>
          <li><a href="javascript:;">欢喜云</a></li>
          <li><a href="javascript:;">应用下载</a></li>
          <li><a href="javascript:;">官方论坛</a></li>
        </ul>
      </div>
    </div>
    <div class="nav-sub">
      <div class="nav-sub-wrapper">
        <div class="container">
          <ul class="nav-list">
            <li><a href="javascript:;">首页</a></li>
            <router-link to="/phone" exact tag="li" activeClass="active">
              <a>手机</a>
            </router-link>
            <li><a href="javascript:;">“足迹系列”手感膜</a></li>
            <router-link to="/" exact tag="li" activeClass="active">
              <a>官方配件</a>
            </router-link>
            <li><a href="javascript:;">周边产品</a></li>
            <li><a href="javascript:;">第三方配件</a></li>
            <li><a href="javascript:;">全部商品</a></li>
            <li><a href="javascript:;">服务</a></li>
          </ul>
        </div>
      </div>
    </div>
  </div>
</template>
<script>
  import CarPanel from '@/components/car-panel'

  export default {
    data() {
      return {
        ok: false,
        name: '',
      }
    },
    components: {
      CarPanel
    },
    created() {
      this.name = sessionStorage.getItem("userName");
      this.ok = sessionStorage.getItem("isLogin");
    },
    methods: {
      logout() {
        // 退出登录
        this.$axios
          .post(`/hammeruser/loginout`, 'id=' + sessionStorage.getItem("id")+"&token="+sessionStorage.getItem("token"))
          .then(({data}) => {
            sessionStorage.clear();
            this.$router.push("/")
            location.reload()
          })
      }
    }
  }
</script>
