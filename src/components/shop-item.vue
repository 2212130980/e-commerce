<template>
  <div class="item">
    <div>
      <div class="item-img"><img :alt="item.name" :src="item.productImg" style="opacity: 1;"></div>
      <h6>{{item.productName}}</h6>
      <h3>{{item.name_title}}</h3>
      <div class="item-btns clearfix">
        <span class="item-gray-btn"><router-link
          :to="{name: 'Item', query: {itemId:item.id}}">查看详情</router-link> </span>
        <span @click="addCarPanelHandle(item)" class="item-blue-btn" v-show="item.isDelete==0&&item.productRepertory>0">加入购物车 </span>
        <span class="item-blue-btn" style="background: red;border: 0;" v-show="item.productRepertory<=0">库存余额不足 </span>
        <span class="item-blue-btn" v-show="item.isDelete==1&&item.productRepertory!=0"
              style="background: red;border: 0;"> 此商品已下架</span>
      </div>
          ...
          this.$axios
            ...
            .then(({data}) => {
              if (data && data.code === 0) {
                ...
                PubSub.publish('search', data.code)
              } else if (data.code === 1) {
                this.$notify({
                  title: '失败',
                  message: '商品库存不足',
