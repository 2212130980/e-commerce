            ...
            <div class="dialog-inner js-address-add">
              <div class="save-address-box">
                <div class="address-form">
                  ...
                  <div class="module-form-row">
                    <div class="form-item-v3">
                      <input type="text" class="js-verify" placeholder="详细地址" v-model="receive.address">
                      <div class="verify-error"></div>
                    </div>
                  </div>
                  <div class="module-form-row">
                    <div class="form-item-v3">
                      <input type="text" class="js-verify" placeholder="备注" v-model="receive.remake">
                      <div class="verify-error"></div>
                    </div>
                  </div>
                  <div class="module-form-row fn-clear">
                    <input type="checkbox" class="hide" >
                    <span class="blue-checkbox" :class="{'blue-checkbox-on':receive.default}"
  ...
  export default {
    ...
    methods: {
      ...
      submitReceiveHandle() {
        ...
        form.append('userAddress', this.receive.address);
        form.append('phone', this.receive.phone);
        form.append('consignee', this.receive.name);
        form.append('remake', this.receive.remake);
        form.append('checked', this.receive.checked ? '1' : '0');
        this.$axios
          .post(`/hammeruseraddress/save`, form)
          .then(({data}) => {
            console.log(data);
            if (data && data.code === 0) {
              // data.page
              if (this.token == "1") {
                this.$parent.initAddress();
              } else {
                this.$parent.init();
              }
              this.$emit('close')
