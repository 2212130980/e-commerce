          ...
          <div class="box-inner js-checkout-address-panel ">
            <div class="address-common-table js-multiple-address-panel">
              <ul class="address-item-list clear js-address-item-list">
                <li class="js-choose-address " :class="{'selected-address-item':info.checked==1?true:false}"
                    v-for="info,index in receiveInfo" @click="selectedAddress(info)">
                  <div class="address-item">
                    <div class="name-section"> {{info.consignee}}</div>
                    <div class="mobile-section">{{info.phone}}</div>
                    <div class="detail-section">{{info.userAddress}}<br> {{info.remake}}</div>
                  </div>
          ...
          <div class="invoice-detail"> 发票抬头：
            <div class="radio-box">
              <label>
                ...
                <span class="blue-radio" :class="{'blue-radio-on':invoice.personal}"
                      @click="checkedInvoice(true)"><a></a></span> 个人
              </label>
              <label>
                <input type="radio" class="hide">
                <span class="blue-radio" :class="{'blue-radio-on':!invoice.personal}"
                      @click="checkedInvoice(false)"><a></a></span> 单位
              </label>
            </div>
      ...
      <div class="gray-box">
        ...
        <div class="box-inner ui-goods-cart">
          ...
          <div class="cart-table">
            <div class="cart-group js-cart-group" v-for="item,index in indentList">
              <div class="cart-items" v-for="items in item.hammerProductEntities">
                <div class="items-thumb">
                  <a href="javascript:;" target="_blank"><img
                    :src="items.productImg"></a>
                </div>
                <div class="name hide-row">
                  <div class="name-cell">
                    <a href="http://www.smartisan.com/shop/#/item/100027501"
                       :title="items.productName" target="_blank">{{items.productName}}</a>
                  </div>
                </div>
                <div class="subtotal">
                  <div class="subtotal-cell"> ¥ {{items.price*item.cartNum}}.00</div>
                </div>
        ...
        <div class="box-inner">
          <div class="order-discount-line">
            <p> 商品总计： <span>¥ {{totalMoney}}.00</span></p>
        ...
        <div class="box-inner">
          <div class="last-payment clear">
            <span class="jianguo-blue-main-btn big-main-btn payment-blue-bt fn-right js-checkout"
                  @click="submitOrderHandle"> <a>提交订单</a> </span> <span class="prices fn-right">应付金额： <em>¥ {{checkedTotle+freight}}.00</em></span>
          </div>
        </div>
      </div>
    </div>
    <address-pop v-if="popShow" :oldReceive="oldReceive" :token="token" @close="closePop"></address-pop>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    components: {
      addressPop, headerNav
    },
    methods: {
      selectedAddress(info) {
        // alert(info.checked=="1")
        if (info.checked == "0") {
      ...
      submitOrderHandle() {
        ...
        let outTradeNo = "";  //订单号
        let date = new Date();
        for (let i = 0; i < 5; i++) //5位随机数，用以加在时间戳后面。
        {
          outTradeNo += Math.floor(Math.random() * 10);
        }
        outTradeNo = date.getFullYear().toString() + (date.getMonth() + 1).toString() +
          date.getDate().toString() + date.getHours().toString() + date.getMinutes().toString()
          + date.getSeconds().toString() + date.getMilliseconds().toString() + outTradeNo;
        ...
        console.log(this.indentList);
        this.$axios
          .post(`/hammerorder/save?userId=`+sessionStorage.getItem("id")+"&userName="+userName+"&cost="+(this.freight+this.totalMoney)
            +"&userAddress="+userAddress+"&orderNumber="+outTradeNo+"&phone="+phone, this.indentList
            ,config)
          .then(({data}) => {
            console.log(data);
            if (data && data.code === 0) {
              this.$router.push({name: 'Payment', query: {orderId: outTradeNo}})
              this.$notify({
                title: '成功',
                message: '订单以生成',
                type: 'success'
      ...
      initAddress() {
        ...
        // console.log(this.$route.params.list+"--------"+this.$route.params.ids);
        let form = new FormData();
        form.append('userId', sessionStorage.getItem("id"));
        // form.append('page', this.currentPage);
        this.$axios
          .post(`/hammeruseraddress/list`, form)
          .then(({data}) => {
            if (data && data.code === 0) {
              this.receiveInfo = data.page.list;
            }
          })
      },
      initIndent() {
        ...
        form.append('cartId', this.$route.query.ids);
        this.$axios
          .post(`/hammercarts/list`, form)
          .then(({data}) => {
            console.log(data);
            if (data && data.code === 0) {
              this.indentList = data.page.records;
              this.indentList.forEach((receive) => {
