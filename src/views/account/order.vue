    ...
    <div class="account-order">
      <div class="gray-box">
        ...
        <div class="js-list-inner">
          <div class="box-inner order-cart order-list-cart clear" v-for="order,index in OrderDto.records">
            <div class="gray-sub-title cart-title">
              <span class="date">{{order.createTime | formatDate}}</span>
              <span class="order-id"> 订单号： <a href="javascript:;">{{order.orderNumber}}</a> </span>
              <span class="order-detail"><router-link
                :to="{name: 'Payment', query: {orderId: order.orderNumber}}">查看详情&gt;</router-link> &nbsp;&nbsp;<i
                class="el-icon-error" @click="delOrder(order.id)"></i></span> <span
              class="sub-total">应付总额</span>
              <span class="operation">物流详情</span>
              <span class="num">数量</span>
              <span class="price">单价</span>
            </div>
            <div class="cart">
              <div class="cart-items clear" v-for="item,index in order.orderDetailDtoList">
                <div class="prod-info clear" v-for="items in item.list">
                  <div class="items-thumb">
                    <a href="javascript:;" target="_blank"><img :src="items.productImg"></a>
                  </div>
                  <div class="items-params clear">
                    <div class="name vh-center">
                      <a href="javascript:;" target="_blank" :title="items.title+'（'+items.productName+'）'">{{items.title}}（{{items.productName}}）</a>
                    </div>
                    <div class="detail"></div>
                  </div>
                  <div class="operation">
                    <div class="operation-list"><br>
                      <span v-if="order.logisticsStatus==0" style="color: red;">未付款</span>
                      <span v-if="order.logisticsStatus==1" style="color: #6b95ea;">待发货</span>
                      <span class="order-detail" v-if="order.logisticsStatus==2">
                        已发货
                      </span><span class="order-detail" v-if="order.logisticsStatus==3">
                        待收货
                      </span><span class="order-detail" v-if="order.logisticsStatus==4">
                        已完成
                      </span>
                    </div>
                  </div>
            ...
            <div class="prod-operation">
              <div class="total">¥ {{order.cost}}.00</div>
              <div class="status">
                <router-link :to="{name: 'Payment', query: {orderId:order.orderNumber}}"
                             class="blue-small-btn js-payment-order" v-if="order.payStatus==0">现在付款
                </router-link>
    ...
    methods: {
      initOrder() {
        let param = {
          ...
          "page": this.OrderDto.current == null || this.OrderDto.current == '' ? 1 : this.OrderDto.current
        }
        this.$axios
          .post(`/hammerorder/list`, qs.stringify(param))
          .then(({data}) => {
            console.log(data);
            if (data && data.code === 0) {
              this.OrderDto = data.page;
              // this.orderInfo = data.hammerOrder;
              // this.initIndent()
            }
          })
      },
      delOrder(id) {
        this.$confirm('确认要删除该订单信息吗?', '提示', {
          ...
          this.$axios
            .post(`/hammerorder/delete`, qs.stringify(param))
            .then(({data}) => {
              console.log(data);
              if (data && data.code === 0) {
                this.$message({
