      ...
      <div class="gray-box clear">
        ...
        <div class="box-inner clear">
          <div v-if="receiveInfo.length">
            <div class="address-list-item default-item" v-for="receive,index in addressList">
              <div class="name fn-left">
                <div class="name-cell">{{receive.consignee}}</div>
              </div>
              <div class="detail fn-left">
                <div class="detail-cell">{{receive.userAddress}}</div>
              </div>
              <div class="operation fn-right">
                <div class="operation-cell">
                  <a class="gray-edit-btn js-edit-address" @click="editReceive(receive,index)">修改</a>
                  <span class="red-del-btn js-del-address" @click="del(receive.id)"> <a>删除</a><em>删除</em> </span>
                </div>
              </div>
              <div class="default fn-right">
                <span>{{receive.remake}}</span>
    ...
    export default {
        ...
        methods: {
            ...
            del(id) {
                ...
                form.append('ids', id);
                this.$axios
                    .post(`/hammeruseraddress/delete`, form)
                    .then(({data}) => {
                        if (data && data.code === 0) {
                            console.log(data);
                            this.$message({
