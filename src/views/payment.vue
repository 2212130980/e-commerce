  ...
  <div id="main">
    ...
    <div class="content page-order-payment">
      <div class="gray-box clear">
        ...
        <div class="box-inner payment-checkout-panel clear">
                    <span class="jianguo-blue-main-btn big-main-btn js-payment-order"
                          @click="payNowHandle(orderInfo.id)" v-if="orderInfo.payStatus==0">
    	                <a>现在支付</a>
	    	            </span>
          <span v-else class="is-pay">已完成 </span>
          <span class="prices"> 应付金额：   <em><span>¥ </span>{{orderInfo.cost}} .00</em>
                    </span>
        </div>
      </div>
      <div class="confirm-detail">
        <div class="info-title">订单编号</div>
        <p class="info-detail">{{orderInfo.orderNumber}}</p>
      </div>
      <div class="confirm-detail">
        <div class="info-title">收货信息</div>
        <p class="info-detail">姓名：{{orderInfo.userName}}</p>
        <p class="info-detail">联系电话：{{orderInfo.phone}}</p>
        <p class="info-detail">详细地址：{{orderInfo.userAddress }} </p>
      </div>
      <div class="confirm-detail">
      ...
      <div class="confirm-goods-table">
        <div class="cart-items clear" v-for="item,index in detailDto">
          <div v-for="items in item.list">
            <div class="name fn-left hide-row">
              <div class="name-cell"><br>
                <a href="javascript:;" :title="items.title+'（'+items.productName+'）'"
                   target="_blank">
                  {{items.title}}（{{items.productName}}）</a><br>
              </div>
            </div>
            <div class="subtotal fn-right">
              <div class="subtotal-cell">¥ {{items.price}}.00</div>
            </div>
            <div class="goods-num fn-right">{{item.quantity}}</div>
            <div class="price fn-right">¥ {{item.cost}}.00</div>
          </div>
        </div>
      </div>
       <div class="order-discount-line">
       <p> 商品总计： <span>¥ {{orderInfo.cost}}.00</span></p>
  ...
  export default {
    ...
    methods: {
      payNowHandle(id) {
        ...
        this.$axios.
          post(`/hammerorder/update`,'id='+id+"&payStatus=1&logisticsStatus=1")
          .then(({data})=>{
            if (data && data.code === 0) {
              this.$alert('成功支付' + (this.orderInfo.cost) + '元', '支付', {
      ...
      initOrder() {
         this.$axios
          .post(`/hammerorder/info`, "orderNumber=" + this.$route.query.orderId)
           .then(({data}) => {
            console.log(data);
            if (data && data.code === 0) {
      ...
      initIndent() {
        this.$axios
          .post(`/hammerorderdetail/list`, "orderId=" + this.orderInfo.id)
          .then(({data}) => {
            console.log(data);
            if (data && data.code === 0) {
              this.detailDto = data.detailDto;
            }
          })
      }
    },
