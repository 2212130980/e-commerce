    ...
    <div class="store-content item">
      <div class="item-box">
        <div class="gallery-wrapper">
          <el-page-header @back="goBack" content="详情页面" style="margin-top:  -40px;"></el-page-header>
          ...
          <div class="gallery">
            ...
            <div class="thumb">
              <ul>
                <li class="on">
                  <img :src="product.productImg" width="480px" height="480px">
                </li>
              </ul>
            </div>
          </div>
        ...
        <div class="banner">
          ...
          <div class="sku-custom-title"><br>
            <div class="params-price">
              <span><em>¥</em><i>{{product.price}}</i></span>
            </div>
          ...
          <div class="sku-dynamic-params-panel">
            <div class="sku-dynamic-params clear">
              <span class="params-name" style="color: #000;">库存   {{product.productRepertory}}   件</span>
            ...
            <div class="sku-dynamic-params clear">
              ...
              <div class="params-detail clear">
                <div class="item-num js-select-quantity">
                  ...
                  <span class="up" :class="{'up-disabled':count>=product.limit_num}" @click="addCount">+</span>
                </div>
              </div>
            </div>
          </div>
          <div class="sku-status">
            <div class="cart-operation-wrapper clearfix">
              <span class="gray-title-btn" @click="checkOutHandle" v-show="product.isDelete==0&&product.productRepertory>0"><a>现在购买</a></span>
              <span class="blue-title-btn js-add-cart" v-show="product.isDelete==1&&product.productRepertory>0"
                    style="background: red;border: 0;"><a>此商品已下架</a></span>
              <br> <br>
              <span class="blue-title-btn js-add-cart" @click="addCarPanelHandle(product)"
                    v-show="product.isDelete==0&&product.productRepertory>0"><a>加入购物车</a></span>
              <span class="blue-title-btn js-add-cart" @click="addMyCollect(product)" v-show="product.isDelete==0&&product.productRepertory>0"><a>加入收藏</a></span>
              <br> <br>
              <br> <br>
...
<script>
  ...
  export default {
    ...
    methods: {
      addCount() {
        this.count++;
        if (this.count > this.product.productRepertory) {
          this.$notify({
            title: '警告',
            message: '商品库存不足哦',
            type: 'warning'
      ...
      addCarPanelHandle(item) {
        ...
        this.$axios
          .post(`/hammercarts/save`, form)
          .then(({data}) => {
            if (data && data.code === 0) {
              // this.total = data.page.total;
              // this.$refs.header.carInit
              PubSub.publish('search')
            }
          })
      },
      init() {
        let productId = this.$route.query.itemId;
        var form = new FormData();
        form.append('id', productId);
        // form.append('page', this.currentPage);
        this.$axios
          .post(`/hammerproduct/info`, form)
          .then(({data}) => {
            if (data && data.code === 0) {
              console.log(data.hammerProduct);
              this.product = data.hammerProduct;
              // this.product= data.page.list;
            }
          })
      },
      addMyCollect(data) {
