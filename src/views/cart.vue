    ...
    <div class="store-content">
      <div class="cart-box">
        ...
        <div class="cart-inner">
          <div v-if="cartList.length<=0" class="empty-label">
            <h3>您的购物车中还没有商品</h3>
          ...
          <div v-else>
            ...
            <div class="cart-table">
              <div class="cart-group">
                <!--购物列表-->
                <div class="cart-top-items" v-for="(item,index) in cartList">
                  <div class="cart-items" v-for="items in item.hammerProductEntities">
                    <div class="items-choose">
                      <!--<span class="blue-checkbox-new" :class="{' checkbox-on':item.checked}"
                            @click="checkGoodsHandle(item.id)"><a></a></span>
-->
                      <input type='checkbox' :checked="fruitIds.indexOf(item.id) > -1" name='checkboxinput'
                             class='input-checkbox' @click='checkedOne(item.id)'>

                    </div>
                    <div class="items-thumb">
                      <router-link :to="{name: 'Item', query: {itemId:items.id}}"></router-link>

                      <img :src="items.productImg">
                    </div>
                    <div class="name hide-row">
                      <div class="name-table">
                        <a href="javascript:;" target="_blank">{{items.productName}}</a>
                      </div>
                    </div>
                    <div class="operation">
                      <a class="items-delete-btn" @click="delCarPanelHandle(items.id)"></a>
                    </div>
                    <div class="subtotal">¥ {{items.price*item.cartNum}}.00</div>
                    <div class="item-cols-num"  >
                      <div class="select js-select-quantity">
                        <span class="down" :class="{' down-disabled':!(fruitIds.indexOf(item.id)>=0 && item.cartNum >1)}"
                              @click="fruitIds.indexOf(item.id)>=0 && item.cartNum > 1?subCarPanelHandle(item):''">-</span>
                        <span class="num">{{item.cartNum}}  </span>
                        <span class="up"
                              :class="{'up-disabled':!(fruitIds.indexOf(item.id)>=0 && item.cartNum < items.productRepertory) }"
                              @click=" fruitIds.indexOf(item.id)>=0 && item.cartNum < items.productRepertory? plusCarPanelHandle(item) :'' ">+ </span>
        ...
        <div v-if="cartList.length>0" class="cart-bottom-bg fix-bottom">
          <div class="cart-bar-operation">
            <div>
              <div class="choose-all js-choose-all">
                <!--<span class="blue-checkbox-new" :class="{'checkbox-on':allChecked}"
                      @click="allGoodsCheckHandle(allChecked)"><a></a></span> 全选-->
                <input type='checkbox' class='input-checkbox' :checked="fruitIds.length==cartList.length"
                       @click='checkedAll()'>全选
          ...
          <div class="shipping">
            <div class="shipping-box">
              <div class="shipping-total shipping-num">
                <h4 class="">
                  已选择 <i>{{fruitIds.length}}</i> 件商品
                </h4>
                <h5>
                  共计 <i>{{fruitIds.length>0?total:0}}</i> 件商品
                </h5>
              </div>
              <div class="shipping-total shipping-price">
                <h4 class="">
                  应付总额：<span>￥</span><i>{{fruitIds.length>0?totalMoney:0}}</i>
                </h4>
            ...
            <span class="jianguo-blue-main-btn big-main-btn js-checkout" :class="{'disabled-btn':fruitIds<=0}"
                  @click="fruitIds.length > 0 ? checkOutHandle():''"><a>现在结算</a></span>
          </div>
        </div>
      </div>
    </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      plusCarPanelHandle(item) {
        item.cartNum++;
        if (this.fruitIds.indexOf(item.id) != -1) {
          this.total++;
        }
         item.hammerProductEntities.forEach((val, index) => {
          this.totalMoney += val.price;
        })
        // alert(item.hammerProductEntities.price)
      },
      subCarPanelHandle(item) {
        item.cartNum--;
        if (this.fruitIds.indexOf(item.id) != -1) {
          this.total--;
        }
      ...
      checkOutHandle() {
        this.$axios
          .post(`/hammercarts/update?ids=`+this.fruitIds,this.cartList)
          .then(({data}) => {
            console.log(data);
            if (data && data.code === 0) {
                // this.cartList = data.page.records;
                 this.$router.push({name: 'Checkout', query: {ids:this.fruitIds}})
            }
          })
      },
      init() {
      ...
      checkedOne(fruitId) {
        ...
        if (idIndex >= 0) {
          ...
          this.cartList.forEach((receive) => {
            if (receive.id == fruitId) {
        ...
        if (this.fruitIds.length == this.cartList.length) {
          this.checkedAll();
        }
